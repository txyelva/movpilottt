...
@router.get("/douban_tv_domestic", summary="豆瓣热门国产剧", response_model=List[schemas.MediaInfo])
async def douban_tv_domestic(page: Optional[int] = 1,
                             count: Optional[int] = 30,
                             _: schemas.TokenPayload = Depends(verify_token)) -> Any:
    """
    豆瓣热门国产剧
    """
    return await RecommendChain().async_douban_tv_domestic(page=page, count=count)


@router.get("/douban_tv_american", summary="豆瓣热门美剧", response_model=List[schemas.MediaInfo])
async def douban_tv_american(page: Optional[int] = 1,
                             count: Optional[int] = 30,
                             _: schemas.TokenPayload = Depends(verify_token)) -> Any:
    """
    豆瓣热门美剧
    """
    return await RecommendChain().async_douban_tv_american(page=page, count=count)


@router.get("/douban_tv_korean", summary="豆瓣热门韩剧", response_model=List[schemas.MediaInfo])
async def douban_tv_korean_endpoint(page: Optional[int] = 1,
                                    count: Optional[int] = 30,
                                    _: schemas.TokenPayload = Depends(verify_token)) -> Any:
    """
    豆瓣热门韩剧
    """
    return await RecommendChain().async_douban_tv_korean(page=page, count=count)


@router.get("/tmdb_tv_chinese", summary="TMDB热播国产剧", response_model=List[schemas.MediaInfo])
async def tmdb_tv_chinese(page: Optional[int] = 1,
                          _: schemas.TokenPayload = Depends(verify_token)) -> Any:
    """
    TMDB热播国产剧（近3个月内有新集播出的中文剧集）
    """
    return await RecommendChain().async_tmdb_tv_chinese(page=page)


@router.get("/tmdb_tv_american", summary="TMDB热播美剧", response_model=List[schemas.MediaInfo])
async def tmdb_tv_american(page: Optional[int] = 1,
                           _: schemas.TokenPayload = Depends(verify_token)) -> Any:
    """
    TMDB热播美剧（近3个月内有新集播出的英语剧集）
    """
    return await RecommendChain().async_tmdb_tv_american(page=page)


@router.get("/tmdb_tv_korean", summary="TMDB热播韩剧", response_model=List[schemas.MediaInfo])
async def tmdb_tv_korean(page: Optional[int] = 1,
                         _: schemas.TokenPayload = Depends(verify_token)) -> Any:
    """
    TMDB热播韩剧（近3个月内有新集播出的韩语剧集）
    """
    return await RecommendChain().async_tmdb_tv_korean(page=page)
