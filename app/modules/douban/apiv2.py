...
class DoubanApi(metaclass=WeakSingleton):
    ...
    def tv_domestic(self, start: Optional[int] = 0, count: Optional[int] = 20,
                    ts=datetime.strftime(datetime.now(), '%Y%m%d')):
        """
        热门国产剧
        """
        return self.__invoke_recommend(self._urls["tv_domestic"],
                                       start=start, count=count, _ts=ts)

    async def async_tv_domestic(self, start: Optional[int] = 0, count: Optional[int] = 20,
                                ts=datetime.strftime(datetime.now(), '%Y%m%d')):
        """
        热门国产剧（异步版本）
        """
        return await self.__async_invoke_recommend(self._urls["tv_domestic"],
                                                   start=start, count=count, _ts=ts)

    def tv_american(self, start: Optional[int] = 0, count: Optional[int] = 20,
                    ts=datetime.strftime(datetime.now(), '%Y%m%d')):
        """
        热门美剧
        """
        return self.__invoke_recommend(self._urls["tv_american"],
                                       start=start, count=count, _ts=ts)

    async def async_tv_american(self, start: Optional[int] = 0, count: Optional[int] = 20,
                                ts=datetime.strftime(datetime.now(), '%Y%m%d')):
        """
        热门美剧（异步版本）
        """
        return await self.__async_invoke_recommend(self._urls["tv_american"],
                                                   start=start, count=count, _ts=ts)

    def tv_korean(self, start: Optional[int] = 0, count: Optional[int] = 20,
                  ts=datetime.strftime(datetime.now(), '%Y%m%d')):
        """
        热门韩剧
        """
        return self.__invoke_recommend(self._urls["tv_korean"],
                                       start=start, count=count, _ts=ts)

    async def async_tv_korean(self, start: Optional[int] = 0, count: Optional[int] = 20,
                              ts=datetime.strftime(datetime.now(), '%Y%m%d')):
        """
        热门韩剧（异步版本）
        """
        return await self.__async_invoke_recommend(self._urls["tv_korean"],
                                                   start=start, count=count, _ts=ts)
