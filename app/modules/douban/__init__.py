...
class DoubanModule(_ModuleBase):
    ...
    def tv_domestic(self, page: int = 1, count: int = 30) -> List[MediaInfo]:
        """
        获取豆瓣热门国产剧
        """
        infos = self.doubanapi.tv_domestic(start=(page - 1) * count,
                                           count=count)
        if infos and infos.get("subject_collection_items"):
            return [MediaInfo(douban_info=info) for info in infos.get("subject_collection_items")]
        return []

    async def async_tv_domestic(self, page: int = 1, count: int = 30) -> List[MediaInfo]:
        """
        获取豆瓣热门国产剧（异步版本）
        """
        infos = await self.doubanapi.async_tv_domestic(start=(page - 1) * count,
                                                       count=count)
        if infos and infos.get("subject_collection_items"):
            return [MediaInfo(douban_info=info) for info in infos.get("subject_collection_items")]
        return []

    def tv_american(self, page: int = 1, count: int = 30) -> List[MediaInfo]:
        """
        获取豆瓣热门美剧
        """
        infos = self.doubanapi.tv_american(start=(page - 1) * count,
                                           count=count)
        if infos and infos.get("subject_collection_items"):
            return [MediaInfo(douban_info=info) for info in infos.get("subject_collection_items")]
        return []

    async def async_tv_american(self, page: int = 1, count: int = 30) -> List[MediaInfo]:
        """
        获取豆瓣热门美剧（异步版本）
        """
        infos = await self.doubanapi.async_tv_american(start=(page - 1) * count,
                                                       count=count)
        if infos and infos.get("subject_collection_items"):
            return [MediaInfo(douban_info=info) for info in infos.get("subject_collection_items")]
        return []

    def tv_korean(self, page: int = 1, count: int = 30) -> List[MediaInfo]:
        """
        获取豆瓣热门韩剧
        """
        infos = self.doubanapi.tv_korean(start=(page - 1) * count,
                                         count=count)
        if infos and infos.get("subject_collection_items"):
            return [MediaInfo(douban_info=info) for info in infos.get("subject_collection_items")]
        return []

    async def async_tv_korean(self, page: int = 1, count: int = 30) -> List[MediaInfo]:
        """
        获取豆瓣热门韩剧（异步版本）
        """
        infos = await self.doubanapi.async_tv_korean(start=(page - 1) * count,
                                                     count=count)
        if infos and infos.get("subject_collection_items"):
            return [MediaInfo(douban_info=info) for info in infos.get("subject_collection_items")]
        return []
